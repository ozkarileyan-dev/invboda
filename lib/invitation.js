...
export async function findInvitation(token) {
  ...
  const rows = await supabaseRequest(
    `invitation_tokens?select=id,family_id,invited_guest_count,expires_at,revoked_at,families(display_name,guest_names),rsvp_responses(status,confirmed_guest_count,optional_message)&token_hash=eq.${tokenHash}&limit=1`
  );
...
export function invitationPayload(invitation) {
  ...
  return {
    ...
    guestNames: invitation.families.guest_names || null,
